...
const getFallbackProjects = (): Project[] => [
  {
    ...
    bedrooms: 3,
    area: 150,
    description: 'كمبوند سكني راقي في قلب العاصمة الإدارية الجديدة مع جميع الخدمات والمرافق الحديثة',
    image: '/images/placeholder.jpg',
    features: ['مسابح', 'حدائق', 'أمن وحراسة 24 ساعة', 'نادي صحي', 'منطقة تجارية', 'مدارس دولية'],
    deliveryDate: '2025',
  ...
  },
  {
    ...
    bedrooms: 4,
    area: 300,
    description: 'فيلات فاخرة بتصميم عصري في قلب مدينة الشيخ زايد بجميع الخدمات المتميزة',
    image: '/images/placeholder.jpg',
    features: ['حديقة خاصة', 'جراج مغطى', 'تكييف مركزي', 'نظام أمان', 'مسبح خاص'],
    deliveryDate: '2024',
    developer: 'AMG للتطوير العقاري',
    hasFullDetails: true,
    featured: true,
  ...
  },
  {
    ...
    bedrooms: 0,
    area: 100,
    description: 'برج تجاري حديث في قلب القاهرة مع إطلالات استثنائية ومرافق متطورة',
    image: '/images/placeholder.jpg',
    features: ['إطلالة بانورامية', 'مصاعد سريعة', 'أمن متطور', 'مواقف سيارات', 'قاعات اجتماعات'],
    deliveryDate: '2025',
    developer: 'AMG للتطوير العقاري',
    hasFullDetails: true,
    featured: true,
  ...
  },
  {
    ...
    bedrooms: 2,
    area: 120,
    description: 'كمبوند سكني متكامل في مدينة نصر بموقع استراتيجي ومرافق عالية الجودة',
    image: '/images/placeholder.jpg',
    features: ['حدائق واسعة', 'منطقة ألعاب', 'كافيتريا', 'صالة رياضية', 'أمن وحراسة'],
    deliveryDate: '2024',
    developer: 'AMG للتطوير العقاري',
    hasFullDetails: true,
    featured: true,
  ...
  },
  {
    ...
    bedrooms: 3,
    area: 180,
    description: 'شاليهات فاخرة على الساحل الشمالي بإطلالة مباشرة على البحر وجميع المرافق السياحية',
    image: '/images/placeholder.jpg',
    features: ['إطلالة بحرية', 'شاطئ خاص', 'مسابح', 'مطاعم', 'ملاعب رياضية', 'سبا'],
    deliveryDate: '2025',
    developer: 'AMG للتطوير العقاري',
    hasFullDetails: true,
    featured: true,
  ...
  },
  {
    ...
    bedrooms: 3,
    area: 200,
    description: 'شقق فاخرة بإطلالة مباشرة على النيل في أرقى أحياء القاهرة',
    image: '/images/placeholder.jpg',
    features: ['إطلالة على النيل', 'تشطيبات فاخرة', 'مصعد بانورامي', 'حمام سباحة', 'كونسيرج'],
    deliveryDate: '2026',
    developer: 'AMG للتطوير العقاري',
    hasFullDetails: true,
    featured: true,
  ...
  const fetchFeaturedProjects = async () => {
    try {
      ...
      if (data.success) {
        ...
        // عرض المشاريع من قاعدة البيانات أو رسالة إذا كانت فارغة
        setProjects(apiProjects)
        
        // Initialize loading states for images if there are projects
        if (apiProjects.length > 0) {
          const loadingStates = apiProjects.reduce((acc: any, project: Project) => ({ 
            ...acc, 
            [project.id]: true 
          }), {})
        ...
        // في حالة فشل API، استخدم البيانات الاحتياطية
        const fallbackProjects = getFallbackProjects()
        setProjects(fallbackProjects)
        
        const loadingStates = fallbackProjects.reduce((acc: any, project: Project) => ({ 
          ...acc, 
          [project.id]: true 
        }), {})
        setImageLoadingStates(loadingStates)
      ...
      // في حالة حدوث خطأ، استخدم البيانات الاحتياطية بدلاً من عرض رسالة خطأ
      const fallbackProjects = getFallbackProjects()
      setProjects(fallbackProjects)
      ...
      setError('') // امسح رسالة الخطأ لأننا نعرض البيانات الاحتياطية
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8 mb-12">
            {projects.length === 0 ? (
              <div className="col-span-full text-center py-20">
                ...
                <h3 className="text-xl font-semibold text-gray-700 mb-2">لا توجد مشاريع مميزة حالياً</h3>
                <p className="text-gray-500 mb-4">لم يتم إضافة أي مشاريع مميزة بعد</p>
                <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-4 py-2 rounded-lg text-sm">
                  ...
                  يمكنك إضافة مشاريع جديدة من لوحة الإدارة
                </div>
              </div>
            ) : (
